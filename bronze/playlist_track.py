...
df_playlist_track = spark.read.csv(f'{source_path}/playlist_track.csv', header = True, inferSchema = True, sep = ',')

# COMMAND ----------

df_playlist_track.write.mode('overwrite').format('delta').option('mergeSchema', 'true').save(f'{bronze_path}/playlist_track')